...
rootpath = os.getcwd() # root path
sys.path.insert(0, rootpath)
sys.path.insert(0, '{}/script'.format(rootpath))
...
checkpoint = torch.load('{}/models/{}_vit/model.pt'.format(rootpath, model_condition), map_location='cpu')
...
def evaluate(basepath, mov_name, estimation_results=False, visualize=True, saveframes=True, outputfps=15):
    # estimation_results: If results already existed (False:create estimation, True: read estimation results)
    # visualize: If want to visualize the output as videos
    # saveframes: If want to save inidividual frames
    ...
    for frm in frames:
        ...
        if visualize:
            ...
            if saveframes:
                os.makedirs("{}/{}_est".format(basepath, mov_name), exist_ok=True)
                cv2.imwrite("{}/{}_est/frame{}.jpg".format(basepath, mov_name, counter), inputs)
            video.write(inputs)
...
basepath = '{}/demo'.format(rootpath)
mov_name = 'skateboard'
evaluate(basepath, mov_name, visualize=True, saveframes=True, outputfps=15)
